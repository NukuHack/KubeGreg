...
GTCEuStartupEvents.registry('gtceu:machine', event => {
    event.create('nuclear_reactor', 'multiblock')
        ...
        .pattern(definition => FactoryBlockPattern.start()
            ...
            .aisle('  HHH ASB HHH  ', '  HHH  K  HHH  ', '   F       F   ', '   F       F   ', '   F       F   ', '   F       F   ', '   F       F   ', '   F       F   ', '   F       F   ', '  HHH     HHH  ', '  HHH     HHH  ')
            ...
            .where('B', Predicates.abilities(PartAbility.EXPORT_ITEMS).setExactLimit(1))
			
            .where('P', Predicates.blocks(GTBlocks.CASING_STEEL_PIPE.get()))
            .where('O', Predicates.abilities(PartAbility.IMPORT_FLUIDS).setMinGlobalLimited(1)
